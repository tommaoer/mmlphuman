	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_distortion.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, n_contrib, out_color, out_opacity, out_depth, out_feature, out_normal, out_surface_xyz, out_distortion, radii, geomBuffer, binningBuffer, imgBuffer);
}
