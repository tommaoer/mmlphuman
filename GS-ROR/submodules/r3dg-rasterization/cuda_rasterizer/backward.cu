...
renderCUDA(
	...
	const float* __restrict__ dL_dpixels_dist,
	...
	)
{
	...
	float dL_dpixel[C];
	float dL_dpixel_d, dL_dpixel_o, dL_dpixel_f[24], dL_dpixel_dist;
	if (inside)
	{	for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
        dL_dpixel_d = dL_dpixels_d[pix_id];
        dL_dpixel_o = dL_dpixels_o[pix_id];
        dL_dpixel_dist = dL_dpixels_dist[pix_id];
		for (int i = 0; i < S; i++)
...
//                 }
			...
			dL_dweight += (final_D2 + m_d * m_d * final_A - 2 * m_d * final_D) * dL_dpixel_dist;
