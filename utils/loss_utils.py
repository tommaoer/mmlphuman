...
def normal_cosine_loss(normal_pred, normal_gt, mask=None, eps=1e-6):
    ...
    cos_sim = (normal_pred * normal_gt).sum(dim=-1).clamp(-1.0, 1.0)
    loss = 1.0 - cos_sim
    if mask is not None:
        valid = mask.bool()
        if valid.sum().item() > 0:
...
def charbonnier_tv_loss(img, mask=None, eps=1e-3):
    dx = img[:, 1:, :] - img[:, :-1, :]
    dy = img[1:, :, :] - img[:-1, :, :]
    ...
    if mask is not None:
        ...
        my = (mask[1:, :] & mask[:-1, :]).unsqueeze(-1).float()
        if mx.sum().item() > 0:
            dx = (dx * mx).sum() / (mx.sum() * dx.shape[-1])
        else:
            dx = dx.mean()
        if my.sum().item() > 0:
            dy = (dy * my).sum() / (my.sum() * dy.shape[-1])
        else:
            dy = dy.mean()
        return torch.nan_to_num(dx + dy, nan=0.0, posinf=1.0, neginf=1.0)
    return torch.nan_to_num(dx.mean() + dy.mean(), nan=0.0, posinf=1.0, neginf=1.0)
